...
- 时间轴播放进度指示器：黄色光标实时追踪播放位置，单路时间轴和多路时间轴均支持，多路仅高亮当前播放轨道
- 声音提醒：Web Audio API 播放 880→660Hz 双音提示音，检测/离线/告警事件自动触发，localStorage 持久化开关和音量，设置面板可配置
- 摄像头分组折叠视图：网格按分组显示标题栏（分组名 + 在线数/总数），点击折叠/展开，有分组在前无分组在后
...
- 摄像头帧延迟指示器：画面右下角实时延迟显示（ms/s），基于 serverTimestamp 计算，三级颜色（绿≤200ms/黄≤500ms/红）
- 录像 A-B 循环播放：进度条区间高亮，A/B 按钮和 [/]/\ 快捷键，逐帧回看可疑画面
- 录像面板总时长统计：头部显示筛选录像数量、总大小、总时长
- 事件类型可视化标签过滤：替换下拉框为彩色标签按钮（motion 黄/detect 青/offline 红）
- RTSP 连接测试：添加摄像头前验证码流可达性，ffmpeg 探测返回视频编码信息
- 录像一键下载：播放器头部和列表项 hover 显示下载按钮，直接下载原始 MP4
- 录像时间搜索：输入 HH:MM/完整时间快速定位对应录像，自动滚动高亮
- 页面可见性优化：切到后台跳过帧渲染，节省 blob URL 创建/释放开销
- 录像播放器快捷键帮助：? 按钮显示所有键盘操作浮层
- 摄像头帧率不足自动告警：SystemMonitor 连续检测低 FPS 触发 camera:lowfps 事件，AlertEngine 支持告警规则，前端橙色标签 + 通知
- 告警面板帧率不足事件类型：规则创建/编辑支持 camera:lowfps 类型
- 录像日期快速导航：日期选择器旁前后一天切换按钮
- 摄像头画面冻结检测：在线但 10 秒无新帧显示橙色闪烁警告徽标
- 录像导出时长预设：30秒/1分钟/5分钟/全部快速选择按钮
- 事件面板 motion 变动比例可视化：彩色比例条（绿/黄/红）直观显示严重程度

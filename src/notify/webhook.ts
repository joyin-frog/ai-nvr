...
export class WebhookNotifier {
  ...
  private buildPayload(event: EventName, payload: Record<string, unknown>): WebhookPayload {
    ...
    if (event === "motion") {
      ...
      const detections = payload.detections as Array<{ label: string; score: number; box?: { xmin: number; ymin: number; xmax: number; ymax: number }; trackId?: number; trackName?: string }> | undefined;
      detail.detections = detections?.map(d => ({ label: d.label, score: d.score, box: d.box, trackId: d.trackId, trackName: d.trackName }));

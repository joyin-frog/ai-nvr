...
export class AlertStorage {
  ...
  listRules(): AlertRule[] {
    return this.db.query(
      "SELECT id, name, event_type as eventType, camera_id as cameraId, labels, window_seconds as windowSeconds, threshold, cooldown_seconds as cooldownSeconds, enabled FROM alert_rules ORDER BY id"
    ).all() as AlertRule[];
  }
  ...
  getEnabledRules(): AlertRule[] {
    return this.db.query(
      "SELECT id, name, event_type as eventType, camera_id as cameraId, labels, window_seconds as windowSeconds, threshold, cooldown_seconds as cooldownSeconds, enabled FROM alert_rules WHERE enabled = 1"
    ).all() as AlertRule[];
  }
  ...
  addRule(rule: Omit<AlertRule, "id" | "enabled">): number {
    const result = this.db.query(
      "INSERT INTO alert_rules (name, event_type, camera_id, labels, window_seconds, threshold, cooldown_seconds, enabled) VALUES (?, ?, ?, ?, ?, ?, ?, 1) RETURNING id"
    ).get(rule.name, rule.eventType, rule.cameraId, rule.labels, rule.windowSeconds, rule.threshold, rule.cooldownSeconds);
    return (result as { id: number }).id;
  }

  /** 更新规则 */
  updateRule(id: number, updates: Partial<Pick<AlertRule, "name" | "eventType" | "cameraId" | "labels" | "windowSeconds" | "threshold" | "cooldownSeconds" | "enabled">>): boolean {
    const sets: string[] = [];

...
function drawDetectionOverlay(ctx: CanvasRenderingContext2D, width: number, height: number) {
  ...
  if (notifications.length > 0) {
    ...
    for (let i = 0; i < notifications.length; i++) {
      ...
      /** 通知文字 */
      const arrow = n.type === 'enter' ? '→' : n.type === 'leave' ? '←' : n.type === 'line-cross' ? '⚡' : '⏳'
      ...
      ctx.fillStyle = n.type === 'enter' ? 'rgba(0, 150, 136, 0.9)' : n.type === 'leave' ? 'rgba(156, 39, 176, 0.9)' : n.type === 'line-cross' ? 'rgba(255, 111, 0, 0.9)' : 'rgba(255, 152, 0, 0.9)'
      ctx.beginPath()
